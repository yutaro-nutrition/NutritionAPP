...
const OUT_XLSX = path.resolve(ROOT, "recipe_db_main_pork_100_batch1.xlsx");
const OUT_XLSX_ALIAS = path.resolve(ROOT, "recipe_db_main_pork_batch1.xlsx");
const OUT_REPORT = path.resolve(ROOT, "main_pork_generation_report_batch1.json");
...
const CUT_SPECS = {
  pork_momo: { label: "豚もも", display: "豚もも肉", count: 14, fatClass: "low" },
  pork_roast: { label: "豚ロース", display: "豚ロース", count: 14, fatClass: "mid" },
  pork_kata: { label: "豚肩", display: "豚肩肉", count: 14, fatClass: "mid" },
  pork_katarosu: { label: "豚肩ロース", display: "豚肩ロース", count: 14, fatClass: "high" },
  pork_hire: { label: "豚ヒレ", display: "豚ヒレ肉", count: 14, fatClass: "low" },
  pork_mince: { label: "豚ひき肉", display: "豚ひき肉", count: 24, fatClass: "high" },
  pork_bara: { label: "豚バラ", display: "豚バラ", count: 6, fatClass: "high" },
};
...
function makeSpecs(foods, catalog) {
  ...
  const F = {
    pork_momo: pick("豚もも肉", ["＜畜肉類＞　ぶた　［大型種肉］　もも"]),
    pork_roast: pick("豚ロース", ["＜畜肉類＞　ぶた　［大型種肉］　ロース", "＜畜肉類＞　ぶた　［大型種肉］　かたロース"]),
    pork_kata: pick("豚肩肉", ["＜畜肉類＞　ぶた　［大型種肉］　かた　赤肉"]),
    pork_katarosu: pick("豚肩ロース", ["＜畜肉類＞　ぶた　［大型種肉］　かたロース"]),
    pork_hire: pick("豚ヒレ肉", ["＜畜肉類＞　ぶた　［大型種肉］　ヒレ"]),
    pork_mince: pick("豚ひき肉", ["＜畜肉類＞　ぶた　［ひき肉］　生"]),
    pork_bara: pick("豚バラ", ["＜畜肉類＞　ぶた　［大型種肉］　ばら　脂身つき　生", "＜畜肉類＞　ぶた　［大型種肉］　ばら"]),
    onion: pick("たまねぎ"),
    carrot: pick("にんじん"),
    cabbage: pick("キャベツ"),
  ...
  const styleCounts = {};
  for (let i = 0; i < TARGET; i += 1) {
    const id = `MAIN_PORK_${String(i + 1).padStart(3, "0")}`;
  ...
  const minceRatio = specs.filter((s) => s.cutKey === "pork_mince").length / specs.length;
  ...
  return { specs, styleCounts, leanFoodName: F.pork_hire.foodName };
}
...
function main() {
  ...
  for (const spec of specs) {
    ...
    master.push({
      ...
      Notes: `Category=主菜(豚肉); Cut=${spec.cutLabel}; Style=${spec.styleLabel}; 主食なし=true; category_validity=true; Food_Master=${path.basename(foodPath)}; Yield=Yield_Table; Retention=Retention_Table; ingredients=${rows.length}`,
    });
  }

  if (master.length !== TARGET) throw new Error("recipe count mismatch");
  for (let i = 1; i <= TARGET; i += 1) {
    const ex = `MAIN_PORK_${String(i).padStart(3, "0")}`;
  ...
  const minceRatio = Number(((cutCounts.pork_mince || 0) / TARGET).toFixed(3));
