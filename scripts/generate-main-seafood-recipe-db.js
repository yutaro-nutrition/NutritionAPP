...
const OUT_XLSX = path.resolve(ROOT, "recipe_db_main_seafood_100_batch1.xlsx");
const OUT_XLSX_ALIAS = path.resolve(ROOT, "recipe_db_main_seafood_batch1.xlsx");
const OUT_REPORT = path.resolve(ROOT, "main_seafood_generation_report_batch1.json");
...
const CUT_SPECS = {
  tuna_red: { label: "まぐろ赤身", display: "まぐろ赤身", count: 14, fatClass: "low", fishType: "赤身魚" },
  bonito_red: { label: "かつお", display: "かつお", count: 8, fatClass: "low", fishType: "赤身魚" },
  tai_white: { label: "まだい", display: "まだい", count: 14, fatClass: "low", fishType: "白身魚" },
  tara_white: { label: "まだら", display: "まだら", count: 14, fatClass: "low", fishType: "白身魚" },
  saba_blue: { label: "さば", display: "さば", count: 14, fatClass: "high", fishType: "青魚" },
  buri_blue: { label: "ぶり", display: "ぶり", count: 12, fatClass: "high", fishType: "青魚" },
  shrimp: { label: "えび", display: "えび", count: 10, fatClass: "low", fishType: "えび" },
  squid: { label: "いか", display: "いか", count: 8, fatClass: "low", fishType: "いか" },
  shell_asari: { label: "あさり", display: "あさり", count: 3, fatClass: "low", fishType: "貝類" },
  shell_hotate: { label: "ほたて", display: "ほたて", count: 3, fatClass: "low", fishType: "貝類" },
};
...
function retryAdjust(spec, rows, attempt, leanFoodName) {
  ...
  const fats = a.filter((r) => /調合油|ごま油|ロース|肩ロース|バラ|ひき肉/.test(`${r.Ingredient_Name} ${r.Notes}`));
  ...
  if (needF && attempt >= 2) {
    for (const r of a) {
      if (groupOf(r.Notes) === "protein" && /ロース|肩|肩ロース|バラ|ひき肉/.test(r.Ingredient_Name)) {
        r.Ingredient_Name = leanFoodName;
...
function makeSpecs(foods, catalog) {
  ...
  const F = {
    tuna_red: pick("まぐろ赤身", ["＜魚類＞　（まぐろ類）　くろまぐろ　天然　赤身　生"]),
    bonito_red: pick("かつお", ["＜魚類＞　（かつお類）　かつお　春獲り　生", "＜魚類＞　（かつお類）　かつお　秋獲り　生"]),
    tai_white: pick("まだい", ["＜魚類＞　（たい類）　まだい　天然　生"]),
    tara_white: pick("まだら", ["＜魚類＞　（たら類）　まだら　生"]),
    saba_blue: pick("さば", ["＜魚類＞　（さば類）　まさば　生"]),
    buri_blue: pick("ぶり", ["＜魚類＞　ぶり　成魚　生"]),
    shrimp: pick("えび", ["＜えび・かに類＞　（えび類）　バナメイえび　養殖　生"]),
    squid: pick("いか", ["＜いか・たこ類＞　（いか類）　するめいか　生"]),
    shell_asari: pick("あさり", ["＜貝類＞　あさり　生"]),
    shell_hotate: pick("ほたて", ["＜貝類＞　ほたてがい　貝柱　生"]),
    onion: pick("たまねぎ"),
    carrot: pick("にんじん"),
    cabbage: pick("キャベツ"),
  ...
  const styleCounts = {};
  for (let i = 0; i < TARGET; i += 1) {
    const id = `MAIN_SEAFOOD_${String(i + 1).padStart(3, "0")}`;
  ...
  return { specs, styleCounts, leanFoodName: F.tuna_red.foodName };
}
...
function main() {
  ...
  for (const spec of specs) {
    ...
    master.push({
      ...
      Notes: `Category=主菜(魚介類); FishType=${CUT_SPECS[spec.cutKey].fishType}; Species=${spec.cutLabel}; Style=${spec.styleLabel}; 主食なし=true; category_validity=true; Food_Master=${path.basename(foodPath)}; Yield=Yield_Table; Retention=Retention_Table; ingredients=${rows.length}`,
    });
  }

  if (master.length !== TARGET) throw new Error("recipe count mismatch");
  for (let i = 1; i <= TARGET; i += 1) {
    const ex = `MAIN_SEAFOOD_${String(i).padStart(3, "0")}`;
  ...
  for (const s of specs) {
    const fishType = CUT_SPECS[s.cutKey].fishType;
    fishTypeCounts[fishType] = (fishTypeCounts[fishType] || 0) + 1;
  }
  ...
  const fishTypeDistributionOk = ["赤身魚", "白身魚", "青魚", "えび", "いか", "貝類"].every((k) => (fishTypeCounts[k] || 0) > 0);
  const categoryValidity = methodCoverageOk && stapleFreeOk && ingredientCountOk && cutDistributionOk && proteinConsecutiveOk && fishTypeDistributionOk;
  ...
  const report = {
    ...
    fish_type_counts: fishTypeCounts,
    ...
    checks: {
      ...
      fish_type_distribution_validity: fishTypeDistributionOk,
