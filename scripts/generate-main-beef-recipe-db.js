...
const OUT_XLSX = path.resolve(ROOT, "recipe_db_main_beef_100_batch1.xlsx");
const OUT_XLSX_ALIAS = path.resolve(ROOT, "recipe_db_main_beef_batch1.xlsx");
const OUT_REPORT = path.resolve(ROOT, "main_beef_generation_report_batch1.json");
...
const CUT_SPECS = {
  beef_momo: { label: "牛もも", display: "牛もも肉", count: 13, fatClass: "low" },
  beef_roast: { label: "牛ロース", display: "牛ロース", count: 13, fatClass: "high" },
  beef_kata: { label: "牛肩", display: "牛肩肉", count: 13, fatClass: "mid" },
  beef_katarosu: { label: "牛肩ロース", display: "牛肩ロース", count: 13, fatClass: "high" },
  beef_hire: { label: "牛ヒレ", display: "牛ヒレ肉", count: 12, fatClass: "low" },
  beef_harami: { label: "牛ハラミ", display: "牛ハラミ", count: 12, fatClass: "high" },
  beef_mince: { label: "牛ひき肉", display: "牛ひき肉", count: 24, fatClass: "high" },
};
...
function retryAdjust(spec, rows, attempt, leanFoodName) {
  ...
  const fats = a.filter((r) => /調合油|ごま油|ロース|肩ロース|ハラミ|ひき肉/.test(`${r.Ingredient_Name} ${r.Notes}`));
  ...
  if (needF && attempt >= 2) {
    for (const r of a) {
      if (groupOf(r.Notes) === "protein" && /ロース|肩ロース|ハラミ|ひき肉/.test(r.Ingredient_Name)) {
        r.Ingredient_Name = "牛ヒレ肉";
...
function mkSteps(spec) {
  ...
  const lines = [
    `${spec.recipeName}の材料を計量し、牛肉は部位に合わせて下処理する。`,
    core[spec.method],
    "野菜と調味料を合わせ、味を調える。",
...
function buildCutSequence() {
  const otherCuts = ["beef_momo", "beef_roast", "beef_kata", "beef_katarosu", "beef_hire", "beef_harami"];
  const seq = [];
  for (let block = 0; block < 25; block += 1) {
    seq.push("beef_mince");
    const start = (block * 3) % otherCuts.length;
    seq.push(otherCuts[start], otherCuts[(start + 1) % otherCuts.length], otherCuts[(start + 2) % otherCuts.length]);
  }
  if (seq.length !== TARGET) throw new Error(`cut sequence size invalid: ${seq.length}`);
  ...
  for (const key of seq) {
    ...
    const alt = Object.keys(remaining).find((k) => remaining[k] > 0 && k !== fixed[fixed.length - 1]);
    if (!alt) throw new Error("unable to finalize cut sequence");
    fixed.push(alt);
    remaining[alt] -= 1;
  }

  const used = {};
  for (const k of fixed) used[k] = (used[k] || 0) + 1;
...
function makeSpecs(foods, catalog) {
  ...
  const F = {
    beef_momo: pick("牛もも肉"),
    beef_roast: pick("牛ロース", ["うし", "リブロース", "サーロイン"]),
    beef_kata: pick("牛肩肉", ["うし", "かた"]),
    beef_katarosu: pick("牛肩ロース", ["うし", "かたロース"]),
    beef_hire: pick("牛ヒレ肉", ["うし", "ヒレ"]),
    beef_harami: pick("牛ハラミ", ["うし", "ばら"]),
    beef_mince: pick("牛ひき肉", ["うし", "ひき肉"]),
    onion: pick("たまねぎ"),
    carrot: pick("にんじん"),
    cabbage: pick("キャベツ"),
  ...
  for (let i = 0; i < TARGET; i += 1) {
    const id = `MAIN_BEEF_${String(i + 1).padStart(3, "0")}`;
  ...
  const minceRatio = specs.filter((s) => s.cutKey === "beef_mince").length / specs.length;
  ...
  return { specs, styleCounts, leanFoodName: F.beef_hire.foodName };
}
...
function main() {
  ...
  for (const spec of specs) {
    ...
    master.push({
      ...
      Notes: `Category=主菜(牛肉); Cut=${spec.cutLabel}; Style=${spec.styleLabel}; 主食なし=true; category_validity=true; Food_Master=${path.basename(foodPath)}; Yield=Yield_Table; Retention=Retention_Table; ingredients=${rows.length}`,
    });
  }

  if (master.length !== TARGET) throw new Error("recipe count mismatch");
  for (let i = 1; i <= TARGET; i += 1) {
    const ex = `MAIN_BEEF_${String(i).padStart(3, "0")}`;
  ...
  const minceRatio = Number(((cutCounts.beef_mince || 0) / TARGET).toFixed(3));
